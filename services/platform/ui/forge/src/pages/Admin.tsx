...
function PlatformPanel({ services }: { services: PlatformService[] }) {
  return (
    <section>
      <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2">
        <Cpu size={14} /> Platform Services
      </h2>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {services.map((svc) => (
          <div
            key={svc.name}
            className={`rounded-xl border px-4 py-3 flex items-start gap-3 bg-white shadow-sm ${
              svc.status === "online" ? "border-gray-200" : "border-red-200"
            }`}
          >
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-10">
        ...
        {loadingSvc ? (
          ...
          <PlatformPanel services={services} />
        )}
